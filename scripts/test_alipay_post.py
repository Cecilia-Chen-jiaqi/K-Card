#!/usr/bin/env python3
"""POST Alipay sandbox form and inspect response."""
import json
import re
import time
import uuid
...
def main():
    ...
    html = req("POST", "/api/pay/submit", {"orderNo": order_no}, token=bt)["data"]

    action = re.search(r'action="([^"]+)"', html, re.I).group(1)
    biz = re.search(r'name="biz_content"\s+value="([^"]*)"', html, re.I).group(1)
    biz = html_unescape(biz)
    print("biz_content preview:", biz[:120])
    ...
    with urllib.request.urlopen(request, timeout=60) as resp:
        ...
        print("final_url", resp.geturl()[:200])
        print("body_len", len(body))
        text = body.decode("utf-8", errors="replace")
        print("body_preview", text[:500])
        if "error" in text.lower() or "异常" in text:
            print("HAS ERROR IN RESPONSE")
