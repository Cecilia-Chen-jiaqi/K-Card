...
def main():
    ...
    print("has_notify_url", "notify_url" in pay_url)
    if "notify_url" in pay_url:
        parsed = urllib.parse.parse_qs(urllib.parse.urlparse(pay_url).query)
        print("notify_url", urllib.parse.unquote(parsed.get("notify_url", [""])[0]))
