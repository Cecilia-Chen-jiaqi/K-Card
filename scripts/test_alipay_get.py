#!/usr/bin/env python3
"""Compare GET vs POST pay redirect for Alipay sandbox."""
import json
import re
import time
import uuid
...
def fetch(url):
    ...
    body = resp.read()
    return resp.status, resp.geturl(), len(body), body[:200]
...
def main():
    ...
    print("GET action URL (wrong - simulates user stuck on gateway.do):")
    s, u, n, p = fetch(action)
    print(" status", s, "final", u[:120], "body_len", n)
    print(" preview", p)
