...
def main():
    ...
    req("POST", "/api/auth/register", {
        ...
        "phone": f"138{TS[-8:]}", "campus": "T",
    })
    req("POST", "/api/auth/register", {
        "username": buyer, "password": "123456",
        "phone": f"139{TS[-8:]}", "campus": "T",
    })
    ...
    goods_id = req("POST", "/api/goods/create", {
    ...
    }, token=seller_token)["data"]["id"]
    order_no = req("POST", "/api/orders/create", {
        "goodsId": goods_id, "quantity": 1, "payType": 1,
    }, token=buyer_token)["data"]["orderNo"]
    pay = req("POST", "/api/pay/submit", {"orderNo": order_no}, token=buyer_token)
    html = pay["data"]
    print("code", pay["code"])
    print("len", len(html))
    print("has_form", "<form" in html.lower())
    print("has_script", "<script" in html.lower())
    action = re.search(r'action="([^"]+)"', html, re.I)
    method = re.search(r'method="([^"]+)"', html, re.I)
    print("form_method", method.group(1) if method else None)
    if action:
        print("action_prefix", action.group(1)[:150])
