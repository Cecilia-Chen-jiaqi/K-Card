#!/usr/bin/env python3
"""Verify pay form uses ngrok return_url."""
import json, re, time, uuid, urllib.request
...
gid=req('POST','/api/goods/create',{'title':'R','description':'x','price':0.01,'stock':1,'groupName':'SEVENTEEN','idolName':'M','quality':'无暇','tradeType':'仅出售','deliveryMode':1,'coverImage':img},token=st)['data']['id']
ono=req('POST','/api/orders/create',{'goodsId':gid,'quantity':1,'payType':1},token=bt)['data']['orderNo']
html=req('POST','/api/pay/submit',{'orderNo':ono},token=bt)['data']
m=re.search(r'return_url=([^&"]+)', html)
ru=urllib.parse.unquote(m.group(1)) if m else ''
print('return_url', ru)
print('PASS' if 'ngrok' in ru and '/api/pay/return' in ru else 'FAIL')
